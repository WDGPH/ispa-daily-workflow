...
from ispa_daily_workflow.normalize import parse_year_value
from ispa_daily_workflow.reference import SchoolReference, parse_school_name_and_id
...
def determine_secondary_birth_cohort(
    ...
    *,
    ...
    assert valid_years
    if min(valid_years) >= 2010:
        return "2010_and_later"
    if max(valid_years) <= 2009:
