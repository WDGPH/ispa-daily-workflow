...
from panorama_compliance.schema import LoadedSchema
...
def read_header(path: Path, file_format: str | None = None) -> list[str]:
    ...
    if suffix == "json":
        ...
        if isinstance(payload, list) and payload and isinstance(payload[0], dict):
            return [str(k).strip() for k in payload[0].keys()]
        if isinstance(payload, dict):
            return [str(k).strip() for k in payload.keys()]
        return []
    ...
    if suffix == "xlsx":
        ...
        row = next(sheet.iter_rows(min_row=1, max_row=1, values_only=True), tuple())
        workbook.close()
    ...
    *,
    ...
    if suffix in {"xlsx", "xls"}:
        ...
        if isinstance(frame, dict):
            ...
            return cast(pl.DataFrame, next(iter(frame.values())))
        return frame
